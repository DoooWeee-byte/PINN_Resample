...
class PhysicsInformedNN():
    def __init__(self, X_u, u, X_b_l, X_b_r, X_f, layers, lb, ub, nu, X_f_test):
        ...
        # 划分验证集和训练集，训练集为X_f_train, 测试集为X_f_val
        '''
        num = len(X_f)
        num_val = int(0.75 * num)
        self.X_f_val = self.X_f[num_val:, :]
        self.X_f_train = self.X_f[:num_val, :]
        '''
    ...
    def validate(self, epoch):
        self.dnn.eval()
        x_val = torch.tensor(self.X_f_val[:, 0:1], requires_grad=True).float().to(device)
        t_val = torch.tensor(self.X_f_val[:, 1:2], requires_grad=True).float().to(device)
        f_pred = self.net_f(x_val, t_val)
        loss_f = torch.mean(f_pred ** 2)
    ...
    def resample_space(self):
        ...
        '''
        self.epoch_adam = self.epoch_adam / 2
        self.epoch_lbfgs = self.epoch_lbfgs / 2
        
        self.epoch_adam = max(int(self.epoch_adam), 10)
        self.epoch_lbfgs = max(int(self.epoch_lbfgs), 1)
        '''
...
X_f_train = lb + (ub - lb) * lhs(2, N_f)  # 采样点，采样空间在(-1, 0)和(1, 0.99)之间
...
# Training
model = torch.load("resample_space/model_resample.pth")

u_pred, f_pred = model.predict(X_star)

error_u = np.linalg.norm(u_star - u_pred, 2) / np.linalg.norm(u_star, 2)  # Error u: 9.414121e-03
